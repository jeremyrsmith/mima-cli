...
case class Main(
  ...
  def run(): Int = {
    ...
    val problems = new MiMaLib(classpath).collectProblems(oldBin, newBin, Nil).flatMap(formatter.formatProblem)
...
object Main {
  ...
  def main(args: Array[String]): Unit =
    try System.exit(parseArgs(args.toList, Main()).run()) catch {
  ...
  private def parseArgs(remaining: List[String], current: Main): Main = remaining match {
    ...
    case ("-cp" | "--classpath") :: cpStr :: rest =>
      parseArgs(rest, current.copy(classpath = cpStr.split(File.pathSeparatorChar).toSeq.map(new File(_))))

    case ("-f" | "--forward-only") :: rest =>
      parseArgs(rest, current.copy(formatter = current.formatter.copy(showForward = true, showBackward = false)))

    case ("-b" | "--backward-only") :: rest =>
      parseArgs(rest, current.copy(formatter = current.formatter.copy(showForward = false, showBackward = true)))

    case ("-j" | "--bytecode-names") :: rest =>
      parseArgs(rest, current.copy(formatter = current.formatter.copy(useBytecodeNames = true)))

    case ("-v" | "--verbose") :: rest =>
      parseArgs(rest, current.copy(formatter = current.formatter.copy(showDescriptions = true)))

    case ("-g" | "--include-generics") :: rest =>
      parseArgs(rest, current.copy(formatter = current.formatter.copy(showIncompatibleSignature = true)))
